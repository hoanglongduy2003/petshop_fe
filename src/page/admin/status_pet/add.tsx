...
const AddStatusPetAdmin: React.FC = () => {
  const [form] = Form.useForm();

  const [createStatusContact] = useCreateStatuspetMutation();

  const navigate = useNavigate();

  const { refetch } = useGetAllstatusPetQuery();
  ...
  return (
    <>
      <h2 className="title-appoiment">Thêm trạng thái thú cưng</h2>
      <Form
        form={form}
        name="validateOnly"
        layout="vertical"
...
export default AddStatusPetAdmin;
